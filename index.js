...
console.log('Task 5:', addReview(reviews, 'Dani', 100, 'this is cool'));
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 6: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
...
function getReviewByRating(arrays, rating) {
  let filteredArray = [];
  for (let i = 0; i < arrays.length; i++) {
    Math.floor(arrays[i].rating) === rating ? filteredArray.push(arrays[i]) : false;
  }
  return filteredArray;
}
console.log('STRETCH ', getReviewByRating(reviews, 4));
...
function getLongReviews(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    let length = array[i].feedback.split(' ').length;
    length > 15 ? filteredArray.push(array[i]) : false;
  }
  return filteredArray;
}
...
function carMaker(odoMiles) {
  /* code here */
  const car = {
    driven: odoMiles,
    drive: function (distance) {
     return this.driven + distance;
      
    }
  }
  return car;
}
const newWagon = carMaker(15);
console.log('STRETCH 3:', newWagon.drive(33));
